function [bodeOut, Fs_pade] = model_3d_pade(dataIn, h, padeOrder, ...
    ...
    % Calls
    %
    %   [bodeOut, Fs_pade] = model_3d_pade(dataIn, h): take the Bode 
    ...
    %
    %   [bodeOut, Fs_pade] = model_3d_pade(dataIn, h, seriesOrder): take 
    ...
    %
    %   [bodeOut, Fs_pade] = model_3d_pade(dataIn, h, seriesOrder, 
    %   padeOrder): take the Bode diagram for 3D model of the heat transfer 
    ...
    %   is taken as padeOrder;
    %
    %   [bodeOut, Fs_pade] = model_3d_pade(__, optons): take the optional 
    %   arguments.
    %
    % Inputs
    % 
    ...
    %% Pade approximation for the 3D rear face model 
    ...
    for n = 0:seriesOrder % Serie en y
        ...
        for m = 0:seriesOrder % Serie en z
            ...
            Fs_pade_ev{1} = Fs_pade_ev{1} + Fs_eval * ...
                (Y/Nalpha(n+1))*(Z/Mbeta(m+1))*int_Z(m+1)*int_Y(n+1);
            
            % Transfer function
            Fs_pade{n+1, 1} = tf(N.coef,D.coef) * ...
                (Y/Nalpha(n+1))*(Z/Mbeta(m+1))*int_Z(m+1)*int_Y(n+1);

        end
    end

    %% Pade approximation for the 3D front face model 
    ...
    for n = 0:seriesOrder % Serie en y
        ...
        for m = 0:seriesOrder % Serie en z
            ...
            % Add the result transfer functions
            Fs_pade_ev{2} = Fs_pade_ev{2} + Fs_eval * ...
                (Y/Nalpha(n+1))*(Z/Mbeta(m+1))*int_Z(m+1)*int_Y(n+1);
            
            % Transfer function
            Fs_pade{n+1, 2} = tf(N.coef,D.coef) * ...
                (Y/Nalpha(n+1))*(Z/Mbeta(m+1))*int_Z(m+1)*int_Y(n+1);
