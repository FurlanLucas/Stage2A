...
hx1 = 10;                 % [W/m²K] Coefficient de transfert thermique (x);
...
saveas(fig, figDir+"\eqTrans_pertesX.png");
title("Solution d'\'{e}quation transcedente", ....
...
for k = 1:length(Mo) % Change l'ordre de l'approximation
    ...
    for n = 0:No(k) % Serie en y
        ...
        for m = 0:Mo(k) % Serie en z
            ...
            % Résolution en 1D de F(alpha, beta)
            gamma = sqrt(1j*w/a_x + alpha(n+1)^2 + beta(m+1)^2);
            C = lambda_x*gamma.*sinh(e*gamma);
            B = sinh(e*gamma) ./ (lambda_x*gamma);
            A = cosh(e*gamma);
            Fs_th = 1./(C + A*(hx1+hx2) + B*hx1*hx2);
...
saveas(fig, figDir+"\1D_et_3D_pertesX.png");
    ...
    for n = 0:No(k) % Serie en y
        ...
        for m = 0:Mo(k) % Serie en z
            ...
            gamma = sqrt(1j*w/a_x + alpha(n+1)^2 + beta(m+1)^2);
             ...
             % Polinômes en xi
            A = [(lambda_x^2)/(2*e^2), (hx1+hx2)*lambda_x/(2*e), ...
                hx1*hx2/2];
            B = [-(lambda_x^2)/(2*e^2), (hx1+hx2)*lambda_x/(2*e), ...
                -hx1*hx2/2];
            C = [lambda_x/e, 0];
            ...
            N = changeVariable(N(mod(fliplr(1:length(N)),2)==0), ...
                [e^2/a_x (alpha(n+1)*e)^2 + (beta(m+1)*e)^2]);
            D = changeVariable(D(mod(fliplr(1:length(D)),2)==0), ...
                [e^2/a_x (alpha(n+1)*e)^2 + (beta(m+1)*e)^2]);
...
xlabel("Fr\'{e}quences (rad/s)", Interpreter='latex', FontSize=17);
fig.Position = [341 162 689 418];
saveas(fig, figDir+"\padeApprox3D_pertesX.png");
sgtitle({"R\'{e}ponse en fr\'{e}quence de $F_{3D}(s)$", ...
