function [bodeOut, Fs_taylor] = model_2d_taylor(sysData, h, taylorOrder, ...
    ...
    % Calls
    %
    %   [bodeOut, Fs_taylor] = model_2d_taylor(dataIn, h): take the Bode 
    %   diagram for 2D model of the heat transfer using the heat transfert 
    %   coefficient h. The series order is fixed to be 6 and the polynomial 
    %   approximation order to be 10;
    %
    %   [bodeOut, Fs_taylor] = model_2d_taylor(dataIn, h, seriesOrder): take 
    ...
    %
    %   [bodeOut, Fs_taylor] = model_2d_taylor(dataIn, h, seriesOrder, 
    %   taylorOrder): take the Bode diagram for 2D model of the heat transfer 
    ...
    %   taylorOrder;
    %
    %   [bodeOut, Fs_taylor] = model_2d_taylor(__, optons): take the optional 
    %   arguments.
    %
    % Inputs
    % 
    ...
    for n = 0:seriesOrder % Serie in r
        ...
        Fs_taylor_ev{1} = Fs_taylor_ev{1} + Fs_eval * (R/Malpha(n+1))*int_R(n+1);
        
        % Transfer function
        Fs_taylor{n+1, 1} = tf(N.coef,D.coef) * (R/Malpha(n+1))*int_R(n+1);
    end
    ...
    for n = 0:seriesOrder % Serie en r
        ...
        % % Add the result transfer functions
        Fs_taylor_ev{2} = Fs_taylor_ev{2} + Fs_eval * (R/Malpha(n+1))*int_R(n+1);
        
        % Transfer function
        Fs_taylor{n+1, 2} = tf(N.coef,D.coef) * (R/Malpha(n+1))*int_R(n+1);
    end
